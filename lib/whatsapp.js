    ...
    async startSession(userId, sessionId) {
        ...
        sock.ev.on("connection.update", async (update) => {
            ...
            if (connection === "open") {
                ...
                if (this.bot) {
                    try {
                        await this.bot.telegram.sendMessage(uid, `✅ <b>Koneksi whatsapp Berhasil tersambung</b>\nSesi ${sessionId} siap digunakan.`, { parse_mode: 'HTML' });
                    } catch (e) {}
                }
                ...
                if (statusCode !== DisconnectReason.loggedOut) {
                    ...
                    if (this.bot) {
                        try {
                            await this.bot.telegram.sendMessage(uid, `⚠️ <b>Sesi ${sessionId} Terputus/Logout</b>`, { parse_mode: 'HTML' });
                        } catch (e) {}
                    }

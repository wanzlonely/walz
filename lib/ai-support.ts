...
export async function handleAiSupport(conversationId: string, triggerMessageId: string) {
  ...
  try {
    ...
    if (!verdict) failureReason = `Respons AI tidak valid: ${raw.slice(0, 150)}`;
  } catch (err: any) {
    verdict = null;
    failureReason = err?.message || 'Kesalahan tidak diketahui';
    console.error('[ai-support] askGroq failed:', failureReason);
  }

  if (!verdict) {
    await escalate(
      conv,
      `Asisten AI gagal memproses pesan (${failureReason || 'tidak diketahui'})`,
      'normal',
      lastText
    );
    return;

...
// Data Produk Lengkap (Game + Pulsa + Panel)
const PRODUCTS = {
...
function selectOption(element) {
    // Mencari parent .options-grid yang benar, termasuk di dalam section-card
...
// Logika Halaman Index (Search dan Render)
...
function setupGamePage() {
    ...
    if (game.needsServerId) {
        ...
        `;
        document.getElementById('server-id').addEventListener('input', updateGameSummary);
    } else {
        serverIdContainer.innerHTML = '';
    }
    ...
    document.getElementById('user-id').addEventListener('input', updateGameSummary);
    document.getElementById('whatsapp-number').addEventListener('input', updateGameSummary);
...
function setupCartPage() {
    ...
    if (!cartSummaryCard || !payButton || !type) {
        // Handle case where cart is loaded directly without params
        if(cartSummaryCard) cartSummaryCard.innerHTML = `<h3>Terjadi Kesalahan</h3><p style="text-align: center; color: var(--text-light);">Data pesanan tidak ditemukan. Silakan kembali ke halaman utama.</p>`;
    ...
    try {
        ...
        cartSummaryCard.innerHTML = `
            ...
            <p style="text-align: center; color: var(--text-light);">Data pesanan tidak valid. ${error.message || 'Silakan kembali ke halaman utama.'}</p>
        `;
        if (payButton) payButton.style.display = 'none';
        if (paymentInfoSection) paymentInfoSection.style.display = 'none';
        console.error("Cart Setup Error:", error);
        return;
    }
    ...
    // Render Payment Info
    let paymentContent = '';
    if (details.paymentQR) {
        paymentContent = `
            <h4>Scan untuk Bayar</h4>
            <img src="${details.paymentQR}" alt="QR Code" class="qr-code-image">
...
Silakan scan kode QR di atas untuk melakukan pembayaran. Setelah berhasil, klik **Bayar Sekarang**.</div>
        `;
    } else if (details.paymentNumber) {
        paymentContent = `
            <h4>Transfer ke ${details.paymentName}</h4>
            <img src="${details.paymentImg}" alt="${details.paymentName}" class="payment-image">
            <div class="payment-caption">A.N. ${details.paymentHolder}</div>
            <div class="payment-number">${details.paymentNumber}</div>
            <button class="copy-button" onclick="copyToClipboard('${details.paymentNumber}')">Salin Nomor</button>
        `;
    }
...
function setupPanelPage() {
    ...
    // Event Listeners
    userNameInput.addEventListener('input', updatePanelSummary);
    whatsappInput.addEventListener('input', updatePanelSummary);
    document.getElementById('panel-product-list').addEventListener('click', updatePanelSummary);
    document.getElementById('payment-list').addEventListener('click', updatePanelSummary);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (document.querySelector('.game-grid-custom')) {
        ...
        setupGamePage();
        document.getElementById('product-list').addEventListener('click', updateGameSummary);
        document.getElementById('payment-list').addEventListener('click', updateGameSummary);

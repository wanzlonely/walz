...
// Data Produk (DIPASTIKAN LENGKAP DAN SAMA DENGAN DATA ASLI ANDA)
const PRODUCTS = {
    "free-fire": [
    ...
    "panel": [
        { name: "Paket 1 (Lite)", price: 10000, desc: "1 CPU, 256 MB RAM, 1 GB Storage" },
        { name: "Paket 2 (Medium)", price: 20000, desc: "2 CPU, 512 MB RAM, 2 GB Storage" },
        { name: "Paket 3 (Pro)", price: 35000, desc: "4 CPU, 1 GB RAM, 5 GB Storage" }
    ]
};
...
const GamePage = {
        ...
        if (products) {
            products.forEach(product => {
                ...
                productDiv.innerHTML = `
                    ${badgeHtml}
                    <i class="fas fa-gem icon visually-hidden"></i>
    ...
    handleConfirmation() {
        document.getElementById('confirm-button').addEventListener('click', () => {
            ...
            const paymentId = selectedPaymentCard ? selectedProductCard.dataset.id : ''; // Perbaikan: Ambil paymentId dari payment card
            const userId = document.getElementById('user-id').value;
            const serverId = this.game.needsServerId ? document.getElementById('server-id').value : '';
            const whatsappNumber = document.getElementById('whatsapp-number').value;
            
            // Perbaikan: Ambil paymentId dari payment card
            const paymentIdFinal = selectedPaymentCard ? selectedPaymentCard.dataset.id : '';

            if (productId && paymentIdFinal && userId && whatsappNumber) {
                const url = `cart.html?game_key=${this.game.key}&product_id=${productId}&payment_id=${paymentIdFinal}&user_id=${userId}&server_id=${serverId}&whatsapp_number=${whatsappNumber}&voucher_applied=${isVoucherApplied}`;
...
const PulsaPage = {
    renderPulsaProducts() {
        ...
        if (!selectedOperator) {
            pulsaListContainer.innerHTML = '<div class="placeholder-message">Pilih operator terlebih dahulu di langkah 1.</div>';
            return;
        }
    ...
    handleConfirmation() {
        document.getElementById('confirm-button').addEventListener('click', () => {
            ...
            const productLabel = selectedProductCard.dataset.label;
            const productPrice = selectedProductCard.dataset.price;
            const paymentName = PAYMENTS.find(p => p.id === selectedPaymentCard.dataset.id).name;
            
            // Perbaikan: Diarahkan ke WhatsApp karena pulsa/panel sering diproses manual
    ...
    init() {
        ...
        this.renderPulsaProducts(); // Render placeholder awal
        this.handleConfirmation();
    }
};
...
const PanelPage = {
    ...
    handleConfirmation() {
        ...
        orderButton.addEventListener('click', () => {
            ...
            // Perbaikan: Diarahkan ke WhatsApp

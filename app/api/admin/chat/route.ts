...
const CONV_COLS =
  'id, telegram_id, user_name, status, created_at, updated_at, last_message, last_message_at, last_sender_type, unread_by_owner';
...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (searchParams.get('stats') === '1') {
      const [all, unread, open, closed] = await Promise.all([
        supabaseAdmin
          .from('chat_conversations')
    ...
    const conversations = rows
      ...
      .map((c: any) => ({
        ...c,
        user_name: c.user_name || `User ${c.telegram_id}`,
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (action === 'bulk_reply') {
      ...
      if (!ids.length || !t) {
        return NextResponse.json(
          { error: 'Pilih percakapan dan isi pesan' },
          { status: 400 }
        );
      }
    ...
    if (action === 'mark_all_read') {
      const { data: unread, error: uErr } = await supabaseAdmin
        .from('chat_conversations')
        .select('id')
        .gt('unread_by_owner', 0)
        .limit(1000);
      ...
      const ids = (unread || []).map((c: any) => c.id);
